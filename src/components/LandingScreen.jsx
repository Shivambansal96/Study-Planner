...
function LandingScreen() {
    ...
    function AddTheSubject() {
        ...
        const collectedData= {
            ...
            localStorage.setItem('subjectData', JSON.stringify(finalData));
        }
    ...
    return(
        <div id="landing-screen" className="h-screen w-screen border-2 flex flex-col justify-center items-center" >
            ...
            <div id="results" className=" flex flex-col w-1/3 border-2 py-5" style={{display:''}}>
                ...
                {fullData.map((items, index) => {
                    return (
                        ...
                        <div id="table" className="flex w-full justify-around" key={index}>
                            ...
                            <div className="h-10 w-1/4 border-2 flex justify-center items-center hover:cursor-pointer bg-green-700 rounded-md" onClick={(e) => {updateButton(e, index)}} > 
                                <input type="button" value="+" className=" w-7 hover:cursor-pointer" />
                            </div>

                            <div className="h-10 w-1/4 border-2 flex justify-center items-center bg-red-600 rounded-md" onClick={(e) => {updateButton(e, index)}}>
                                <input type="button" value="-" className=" hover:cursor-pointer" />
                            </div>
                        </div>
                    );
                })}
